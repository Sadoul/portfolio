...
window.MC_SCENE = (function(){
  ...
  /* Ломается блок долго нарочно. Показ идёт один раз за посещение
     вкладки: доломав землю, Стив улетает и не возвращается. При 2.3с
     на блок всё кончалось за шесть секунд — раньше, чем на сцену
     успевали посмотреть. 4.6с даёт около десяти замахов на блок и
     полный показ секунд на двенадцать. */
  const MINE_MS  = 4600;    // сколько ломается один блок
